...
To install the chart with the release name `duplicacy`

```console
helm repo add TrueCharts https://charts.truecharts.org
helm repo update
helm install duplicacy TrueCharts/duplicacy
```
...
To uninstall the `duplicacy` deployment

```console
helm uninstall duplicacy
```
...
```console
helm install duplicacy \
  --set env.TZ="America/New York" \
    TrueCharts/duplicacy
```
...
```console
helm install duplicacy TrueCharts/duplicacy -f values.yaml
```
