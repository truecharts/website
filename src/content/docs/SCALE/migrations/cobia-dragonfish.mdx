...
:::note[Upgrade to Dragonfish]
    ...
    Update as usual through TrueNAS' System Settings > Update page.
...
3. Rerun the migration process and select the backup.

    ```shell title="Rerun Migration"
    sudo bash migrate.sh --skip
    ```

    ``` title="Use the backup copied in the previous step"
