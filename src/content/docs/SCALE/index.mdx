...
- Add the TrueCharts catalog with the following trains selected: `stable`, `premium`, `system` -> [Adding TrueCharts](/scale/#adding-truecharts)
...
- Install `VolumeSnapshots` and `VolSync` **after* doing the above steps if you want [backup and restore functionality](/scale/guides/backup-restore) made available for your apps' PVCs
- Install `MetalLB` if you're an advanced user or want to assign specific IP addresses to your SCALE apps. See [here](/scale/#metallb-installation-and-disabling-integrated-loadbalancer)
- Install `Traefik` if you want [ingress or reverse-proxy](/scale/#traefik-installation-for-ingress--reverse-proxy-support-with-truecharts-apps) support with our apps
- Install `Authelia` if you want to add a [single-sign-on or multi-factor-authentication portal](/scale/authelia-installation) to your apps through integration with `Traefik`
- Install `Blocky` if you want to take advantage of our [preferred DNS solution](/scale/#blocky-dns-provider-for-split-dns-installation-and-guide) to allow for DNS proxy, enhancer and ad-blocking functions with your apps.
