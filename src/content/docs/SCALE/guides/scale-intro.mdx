...
See here our SCALE Getting Started [guide](/scale#requirements).
...
Alternatively, you can also use [HeavyScript](/scale/#heavyscript) to update your apps and even configure it to do so automatically via a CRON job. Examples and instructions are found on the linked page and the HeavyScript Github, also linked on that page.
