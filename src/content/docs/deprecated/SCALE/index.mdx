...
TrueNAS SCALE Apps are considered Deprecated.
We heavily advice using a more mature and solid kubernetes platform such as "TalosOS" instead
...
- Ensure you have completed the above requirements
- Add the TrueCharts catalog with the following trains selected: `stable`, `premium`, `system` -> [Adding TrueCharts](/deprecated/scale/#adding-truecharts)
- Install the following apps from the `system` train with **default** settings: `OpenEBS`, `Prometheus Operator` and `Cert-Manager`

Optionally, you can also install the below apps if you want functionality that they provide:

- Install `Cloudnative-PG Operator` if you plan to use any [apps that rely on CNPG functionality](/general/faq#how-do-i-know-if-an-app-uses-cnpg)
- Install `MetalLB` if you're an advanced user or want to assign specific IP addresses to your SCALE apps. See [here](/deprecated/scale/#metallb-installation-and-disabling-integrated-loadbalancer)
- Install `Traefik` if you want [ingress or reverse-proxy](/deprecated/scale/#traefik-installation-for-ingress--reverse-proxy-support-with-truecharts-apps) support with our apps
- Install `Authelia` if you want to add a [single-sign-on or multi-factor-authentication portal](/deprecated/scale/#authelia-installation) to your apps through integration with `Traefik`
- Install `Blocky` if you want to take advantage of our [preferred DNS solution](/deprecated/scale/#blocky-dns-provider-for-split-dns-installation-and-guide) to allow for DNS proxy, enhancer and ad-blocking functions with your apps
- ~~Install `VolSync` **after** doing the above steps if you want [backup and restore functionality](/deprecated/scale/guides/backup-restore) made available for your apps' PVCs. Then, in the options for OpenEBS which you installed earlier, check the [checkboxes](./img/volumesnapshotclass.png) for `enabled` and `isDefault` under the "volumeSnapshotClass" section.~~
...
Please free to check out our [Introduction to TrueNAS SCALE](/deprecated/scale/guides/scale-intro) guide on some specific information on installing, editing, rollbacks and CLI commands for use with apps on TrueNAS SCALE.

:::

## Getting started using Charts with your own Domain

- Steps Above -> [Minimal Getting Started with SCALE](/deprecated/scale#minimal-getting-started-setup-with-scale)
...
- Everything below (includes the steps listed above and extras like [HeavyScript](/deprecated/scale/#heavyscript), [MetalLB](/deprecated/scale/#metallb-installation-and-disabling-integrated-loadbalancer) and [Authelia](/deprecated/scale/#authelia-installation))
...
4.  Create a dedicated, unencrypted and empty dataset on the apps pool you [created above](https://www.truenas.com/docs/scale/scaleuireference/apps/#choose-a-pool-for-apps-dialog), outside of ix-applications. OpenEBS will create unencrypted datasets for PVC app storage in that dataset, using an encrypted dataset for OpenEBS is thus discouraged and pointless. Please ensure you've chosen the dataset that's **not** inside `ix-applications`. An example "apps" **pool** layout could look like [this](/deprecated/scale/migrations/cobia-dragonfish/#post-migration-steps) where you have a top-level "apps" dataset with "ix-applications" (non-PVC app data) and "pvcApps" (OpenEBS PVC app storage) nested under.
